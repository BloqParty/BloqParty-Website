...
for(const key of Object.keys(fs)) {
    ...
    if(!isBlacklisted && typeof fs[key] === 'function') {
        ...
        promisified[key] = (...args) => {
            console.debug(`FS promisified ${key} (${useFuncKey}) with ${args.length} args`);

            const func = useFunc(...args);

            if(func.then) {
                func.then((...r) => console.debug(`FS promisified ${key} (${useFuncKey}) resolved with ${r.length} args`));
                return func;
            } else {
                console.debug(`FS promisified ${key} (${useFuncKey}) resolved with ${func.length} args (not a promise)`);
