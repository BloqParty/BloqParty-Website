        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    {
                        entry.fullCombo ? (
                            <DetailBlock color="linear-gradient(135deg, rgb(112,143,255) 0%, rgb(198,128,237) 100%)" title="Full Combo" value="FC" />
                        ) : (
                    ...
                    <DetailBlock icon={icon({name: 'percent'})} title={`Accuracy: ${entry.accuracy}`} value={(parseFloat(entry.accuracy || 0.00)).toFixed(2)} />
                    <DetailBlock icon={icon({name: 'gear'})} title={`Modifiers Used: ${entry.modifiers}`} value={entry.modifiers.trim().split(` `).join(`, `)} />
                </div>
            </div>
        )
    }
